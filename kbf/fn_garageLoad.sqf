...
#include "GarageIDC.inc"

disableSerialization;
params["_point", "_types"];

fnc_Garage_load = {
	//If already the window open, close it by "Cancel" 如果窗口已经打开，请点击“取消”关闭
	...
	_pos = (getPosASL _point) vectorAdd [0,0,1];
	_dir = getDir _point;
	...
	{
	  ["Vehicles exist in the Area. Please clear and try again"] call BIS_fnc_guiMessage;
	};

	//If someone open garage in same place, warn and exit.如果有人在同一位置打开车库，请警告并退出。
	if (missionNamespace getVariable [format["Open_Garage_%1", _pointName], false]) exitwith
	{
		["Someone is already using this garage spawner, please wait"] call BIS_fnc_guiMessage;
	};
	...
	//Wait until garage is ready. 等到车库准备好。
	...
	//Convert types to corresponding number and check vehicle type.将类型转换为相应的数字并检查车辆类型。
	_fitNum = 0;
	_typeNums = [];
	{
	...
	//Select vehicle random following types.选择车辆随机跟随类型。
	if (_fitNum == 0) then {
		{
